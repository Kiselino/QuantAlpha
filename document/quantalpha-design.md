...
**版本:** v1.8.1 · **日期:** 2026-09-04 · **状态:** v1.8 基础上公开 bootcamp 协议/模板（新用户可起步）+ 官方学习指南入库 + 调研访问规则补强；代码行为不变
...
| **教材分层（v1.8，v1.8.1 修订）** | 通用教材 + 学习闭环协议/模板公开（`document/courses/` + `document/courses/bootcamp/`，随仓库分发）；个人执行档案（mastery/摸底/答题记录）留在本地 `docs/bootcamp/`（gitignored） | 公开知识可分享（符合 P5）；新使用者 clone 即能起步学习（v1.8.1 修复原全私有的断链）；个性化进度属个人数据，隔离在本地上传之外 |
...
> 通用教材已入 `document/courses/`（公开）；**v1.8.1 起学习闭环协议/模板公开于 `document/courses/bootcamp/`**，
> 个人学习档案（mastery/摸底/答题记录）在本地 `docs/bootcamp/`（gitignored）。
