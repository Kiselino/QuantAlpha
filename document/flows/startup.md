...
| # | 检查项 | 正常 | 异常引导动作 |
|---|---|---|---|
| 1 | 新用户判定 | 非新用户 | 首次运行引导：login → update-knowledge → run |
...
| ③ 快速模式 | 高价值复用 | 有正事，让 agent 后台跑（历史主题深挖/失败优化/模版生成） | `document/flows/generation.md` §2.3 |

用户也可在会话中随时口头切换模式。

## 5. 主题来源三选一（教学模式/随机模式进入生成前询问）
