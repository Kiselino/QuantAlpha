...
## 回测效率预期（官方课口径 2026-04，防挫败/管理预期）

- **命中率约千分之一**：回测 1000 个 alpha 才可能出现 1 个有效信号；只回测几百个没结果是**正常现象**，建议至少 2000+ 再对方向下结论
- 单次回测 3 分钟属正常范围；程序化批量回测可 24h 运行（在线回测不可）
- 批量回测数千次仍无可提交 alpha → 模板与数据集不匹配（换匹配的模板/字段，不是继续加量）

## 过拟合风险

- **谈过拟合的前提 = 回测数量足够多**（官方口径：至少几千次；一天几百次谈不上过拟合——但也因此代码批量回测者最易过拟合）
...
- **反向操作警告**：新手不要为降低相关性刻意加噪音（= 引入过拟合）；不要在一个 alpha 上无限调参钻牛角尖（改多了 = 过拟合 + 与已有 alpha 高相关）——放一放，有系统性新想法再回来
- **样本外间接可见**：提交前 IS Testing 预留段（留一段不看，提交前验证趋势保持）；提交后日收入 <1.5 USD 基本可断定样本外表现差
