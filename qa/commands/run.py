...
def _load_operators() -> set[str]:
    """算子白名单：平台官方 67 算子全集（与公开 knowledge/operators.md 一致，2026-08-16 API 实测）。"""
    return {
        # 算术
        "abs",
        "log",
...
def cmd_run(
    ...
    if remaining is not None and remaining <= 0:
        print("[run] 平台每日模拟配额已耗尽，明天再试（EST 日界）。")
        return 1
    ...
    def _prog() -> str:
        return f"(已完成 {progress['done']}/{progress['total']})"
    ...
    def _start_sim(
        ...
        cand, vr = pair
        try:
            sim_id = client.simulate(cand.expression, _settings(cfg, cand.settings))
            if not sim_id:
                return (
                    cand,
                    vr,
    ...
    def _poll_sim(
        item: tuple[Candidate, ValidationResult, str, bool],
    ) -> tuple[
        Candidate,
        ValidationResult,
    ...
    def _record_sim_error(
        cand: Candidate, vr: ValidationResult, err: Exception, sim_id: str | None
    ) -> None:
    ...
    def _record_poll_error(
        cand: Candidate, vr: ValidationResult, sim_id: str, err: Exception
    ) -> None:
    ...
    def _finalize(cand: Candidate, vr: ValidationResult, sim: SimulationResult) -> None:
        ...
        progress["done"] += 1
        print(
            f"    {_prog()} {verdict.verdict}: Sharpe={sim.metrics.get('sharpe') or '—'}"
        )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        for start in range(0, len(todo), max_workers):
            ...
            for cand, vr, sim_id, err in pool.map(_start_sim, new_tasks):
                if err is not None:
                    if session_lost["flag"] and isinstance(err, PermissionError):
                        # 会话中断不是终态：不计入已完成，重跑自动续查/重提
                        print(
                            f"    {_prog()} ✗ 模拟失败（会话过期）: {cand.expression}: {err}"
                        )
                        continue
                    progress["done"] += 1
                    print(f"    {_prog()} ✗ 模拟失败: {cand.expression}: {err}")
                    _record_sim_error(cand, vr, err, None)
                    continue
            ...
            if session_lost["flag"]:
                n_remaining = len(todo) - len(results)
                print(
            ...
            for cand, vr, sim, err, sim_id, resumed in pool.map(_poll_sim, poll_tasks):
                if err is not None:
                    ...
                    progress["done"] += 1
                    print(f"    {_prog()} ✗ 模拟失败: {cand.expression}: {err}")
                    _record_poll_error(cand, vr, sim_id, err)
                    continue
                assert sim is not None, "轮询成功但 sim 为 None"
                _finalize(cand, vr, sim)

            # 阶段 2 续：续查 404 回退重新 simulate（罕见路径，复用同一线程池）
            if retries and not session_lost["flag"]:
                for cand, vr, stale_id in retries:
                    store.delete_simulation(stale_id)
                retry_started: list[tuple[Candidate, ValidationResult, str]] = []
                for cand, vr, sim_id, err in pool.map(
                    _start_sim, [(c, v) for c, v, _ in retries]
                ):
                    if err is not None:
                        if session_lost["flag"] and isinstance(err, PermissionError):
                            print(
                    ...
                    retry_started.append((cand, vr, sim_id))
                for cand, vr, sim, err, sim_id, resumed in pool.map(
                    _poll_sim, [(c, v, s, False) for c, v, s in retry_started]
                ):
                    if err is not None:
                        if session_lost["flag"] and isinstance(err, PermissionError):
                            print(
            ...
            if start + max_workers < len(todo):
                try:
                    ...
                    rl = None
                # 平台每日配额（主线程统一读 rate_limits，v1.6 修并发竞态）耗尽 → 提前停止
                if rl and rl.daily_remaining is not None and rl.daily_remaining <= 0:
                    print(
...
def main(paths: QaPaths, cfg: AppConfig, args) -> int:
    ...
    return cmd_run(paths, cfg, args.candidates_file, args.idea, args.concurrency)
